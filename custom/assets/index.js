...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  DIR_EMPTY_NOTE = PARAMS.q ? '没有搜索结果' : DATA.dir_exists ? '文件夹为空' : '上传文件后将创建文件夹';
...
async function ready() {
  ...
  if (DATA.kind === "Index") {
    document.title = `${DATA.href} - DUFS 文件空间`;
    document.querySelector(".index-page").classList.remove("hidden");

    await setupIndexPage();
  } else if (DATA.kind === "Edit") {
    document.title = `编辑 ${DATA.href} - DUFS 文件空间`;
    document.querySelector(".editor-page").classList.remove("hidden");

    await setupEditorPage();
  } else if (DATA.kind === "View") {
    document.title = `查看 ${DATA.href} - DUFS 文件空间`;
    document.querySelector(".editor-page").classList.remove("hidden");
  ...
  if (isDir) {
    ...
    if (DATA.allow_archive) {
      ...
      <div class="action-btn">
        <a class="dlwt" href="${url}?zip" title="将文件夹下载为 ZIP" aria-label="将文件夹下载为 ZIP" download>${ICONS.download}</a>
      </div>`;
    }
  } else {
    actionDownload = `
    <div class="action-btn" >
      <a class="dlwt" href="${url}" title="下载文件" aria-label="下载文件" download>${ICONS.download}</a>
    </div>`;
  }
  if (DATA.allow_delete) {
    if (DATA.allow_upload) {
      actionMove = `<div onclick="movePath(${index})" class="action-btn" id="moveBtn${index}" title="移动并重命名" aria-label="移动并重命名">${ICONS.move}</div>`;
      if (!isDir) {
        actionEdit = `<a class="action-btn" title="编辑文件" aria-label="编辑文件" target="_blank" href="${url}?edit">${ICONS.edit}</a>`;
      }
    }
    actionDelete = `
    <div onclick="deletePath(${index})" class="action-btn" id="deleteBtn${index}" title="删除" aria-label="删除">${ICONS.delete}</div>`;
  }
  if (!actionEdit && !isDir) {
    actionView = `<a class="action-btn" title="查看文件" aria-label="查看文件" target="_blank" href="${url}?view">${ICONS.view}</a>`;
  }
  let actionCell = `
  ...
  document.querySelectorAll("a.dlwt").forEach(link => {
    link.addEventListener("click", async e => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("获取下载令牌失败");
